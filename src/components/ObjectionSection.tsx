    ...
    <div className="container max-w-3xl text-center space-y-10">
      ...
      <div className="grid gap-8 sm:grid-cols-2 text-left">
        <div className="space-y-4">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-primary-bright">Perfect for you</h3>
          {forYou.map((t) => (
            <div key={t} className="flex items-center gap-3">
              <CheckCircle size={18} className="shrink-0 text-primary-bright" />
              <span className="text-muted-foreground">{t}</span>
            </div>
          ))}
        </div>
        <div className="space-y-4">
          <h3 className="text-sm font-semibold uppercase tracking-wider text-primary">You don't need</h3>
          {dontNeed.map((t) => (
            <div key={t} className="flex items-center gap-3">
              <XCircle size={18} className="shrink-0 text-primary" />
              <span className="text-muted-foreground">{t}</span>
            </div>
          ))}
        </div>
